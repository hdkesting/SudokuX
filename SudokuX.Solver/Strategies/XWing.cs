    ...
    public class XWing : ISolver
    {
        ...
        private IList<Conclusion> FindXWing(int digit, ISudokuGrid grid, GroupType first, GroupType second)
        {
            ...
            var groups = grid.CellGroups
                                ...
                                .Where(g => g.Cells.Count(c => !c.HasValue && c.AvailableValues.Contains(digit)) == 2)
                                .ToList();
            if (groups.Count < 2)
            {
            ...
            foreach (var pair in groups.GetAllPairs())
            {
                ...
                var cross1 =
                    pair.Item1.Cells.Where(c => !c.HasValue && c.AvailableValues.Contains(digit))
                        .Select(c => c.ContainingGroups.First(g => g.GroupType == second))
                        .ToList();
                var cross2 =
                    pair.Item2.Cells.Where(c => !c.HasValue && c.AvailableValues.Contains(digit))
                        .Select(c => c.ContainingGroups.First(g => g.GroupType == second))
                        .ToList();
